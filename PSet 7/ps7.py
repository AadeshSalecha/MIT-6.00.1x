...
def filterStories(stories, triggerlist):
    ...
    result1 = []
    for story in result:
        if story not in result1:
             result1.append(story) 
...
def makeTrigger(triggerMap, triggerType, params, name):
    ...
    """
    name_of_trigger = name[:]

    if (triggerType == "TITLE"):
        name = TitleTrigger(params[0])
    if (triggerType == "PHRASE"):
        name =  PhraseTrigger(" ".join(params))
    if (triggerType == "SUBJECT"):
        name = SubjectTrigger(params[0])
    if (triggerType == "SUMMARY"):
        name = SummaryTrigger(params[0])
    if (triggerType == "NOT"):
    ...
    triggerMap[name_of_trigger] = name
    return name
